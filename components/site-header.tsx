...
export function SiteHeader() {
  ...
  return (
    <header className="theme-surface theme-border border-b transition-colors">
      <div className="mx-auto flex h-[72px] max-w-6xl items-center justify-between px-5 md:h-20">
        ...
        <div className="flex items-center gap-4">
          <button type="button" onClick={() => setTheme(dark ? 'light' : 'dark')} aria-label={dark ? 'Switch to light mode' : 'Switch to dark mode'} className="rounded-full p-2 text-foreground hover:bg-card">
          ...
          <button type="button" className="rounded-full p-2 text-foreground hover:bg-card md:hidden" aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open} onClick={() => setOpen(!open)}>
